...
MovieClip.prototype.setLoops = function(l){
    this.loops = l;
    for (i = 0; i < this.children.length; i += 1) {
...
MovieClip.prototype._animate = function () {
    ...
    for(i = 0; i < this.children.length; i += 1)
    {
        if (this.children[i]._animate && this.children[i].playing) {
...
function SVGAnim(data, w, h, fps, params) {
    ...
    this.remove = function(){
        for (var i = 0; i < instance.mc.length; i++) {
            instance.mc[i].stop();
        }
        rem = true;
    }
